...
fn acc_multiple_directives_and_end_pairs_match_generated_macros() {
    let header = std::fs::read_to_string("src/roup_constants.h").expect("failed to read generated header");
    ...
    for (text, macro_name) in &directives {
        ...
        let caps = re.captures(&header).expect(&format!("{} not found in header", macro_name));
    ...
    let kind = roup::acc_directive_kind(dir);
        assert_eq!(kind, expected, "directive '{}' returned {}, header macro = {}", text, kind, expected);
    ...
    let caps = re.captures(&header).expect("ROUP_ACC_DIRECTIVE_PARALLEL not found in header");
    ...
    assert!(!end_dir.is_null(), "acc_parse returned NULL for 'end parallel'");
    let paired = roup::acc_directive_end_paired_kind(end_dir);
    assert_eq!(paired, expected_parallel, "end paired kind {} != expected {}", paired, expected_parallel);
