...
//! ## Learning Objectives
//!
//! - **Enum-based parsing**: Using FromStr trait for type-safe conversions
...
pub fn parse_directive_kind(name: &str) -> Result<DirectiveKind, ConversionError> {
    DirectiveKind::from_str(name).map_err(|_| ConversionError::UnknownDirective(name.to_string()))
}
...
pub fn parse_reduction_operator(op_str: &str) -> Result<ReductionOperator, ConversionError> {
    ReductionOperator::from_str(op_str).map_err(|_| {
        ConversionError::InvalidClauseSyntax(format!("Unknown reduction operator: {op_str}"))
    })
}
...
pub fn parse_schedule_clause(
    ...
    let (modifiers, rest) = if let Some(colon_pos) = content.find(':') {
        ...
        let mods: Vec<ScheduleModifier> = mod_str
            ...
            .filter(|s| !s.is_empty())
            .map(|s| {
                ScheduleModifier::from_str(s).map_err(|_| {
                    ConversionError::InvalidClauseSyntax(format!("Unknown schedule modifier: {s}"))
                })
            })
    ...
    let kind = match parts.first() {
        Some(s) => ScheduleKind::from_str(s).map_err(|_| {
            ConversionError::InvalidClauseSyntax(format!("Unknown schedule kind: {s}"))
        })?,
        None => {
...
pub fn parse_map_clause(
    ...
    let (map_type, items_str) =
        if let Some((type_str, items)) = lang::split_once_top_level(remainder, ':') {
            ...
            let map_type = if type_str_trimmed.is_empty() {
                None
            } else {
                Some(
                    MapType::from_str(&type_str_trimmed.to_ascii_lowercase()).map_err(|_| {
                        ConversionError::InvalidClauseSyntax(format!(
                            "Unknown map type: {type_str_trimmed}"
                        ))
                    })?,
                )
            };
...
pub fn parse_depend_type(type_str: &str) -> Result<DependType, ConversionError> {
    DependType::from_str(type_str).map_err(|_| {
        ConversionError::InvalidClauseSyntax(format!("Unknown depend type: {type_str}"))
    })
}
    ...
    match clause_name {
        ...
        "default" => {
            if let ClauseKind::Parenthesized(ref content) = clause.kind {
                ...
                let kind = DefaultKind::from_str(kind_str).map_err(|_| {
                    ConversionError::InvalidClauseSyntax(format!(
        ...
        "proc_bind" => {
            if let ClauseKind::Parenthesized(ref content) = clause.kind {
                let content = content.as_ref();
                let kind_str = content.trim();
                let proc_bind = ProcBind::from_str(kind_str).map_err(|_| {
                    ConversionError::InvalidClauseSyntax(format!(
