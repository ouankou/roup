...
fn parse_end_directive<'a>(
    ...
    let (input, _) = lexer::skip_space_and_comments(input)?;
    let (rest, raw_directive) = lexer::lex_identifier_token(input)?;
    let collapsed = lexer::collapse_line_continuations(raw_directive);
    let directive = collapsed.as_ref().trim();
    let (rest, clauses) = clause_registry.parse_sequence(rest)?;
