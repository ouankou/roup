...
pub fn generate_header(directives: &[(String, i32)], clauses: &[(String, i32)]) -> String {
    // Generate directive constants
    let mut directive_defs = String::new();
    for (name, num) in directives {
        directive_defs.push_str(&format!("#define ROUP_DIRECTIVE_{:<20} {}\n", name, num));
    }
    directive_defs.push_str(&format!(
        "#define ROUP_DIRECTIVE_UNKNOWN       {}\n",
        UNKNOWN_KIND
    ));
    ...
    for (name, num) in clauses {
        clause_defs.push_str(&format!("#define ROUP_CLAUSE_{:<15} {}\n", name, num));
    }
    clause_defs.push_str(&format!(
        "#define ROUP_CLAUSE_UNKNOWN      {}\n",
        UNKNOWN_KIND
    ));
...
// ============================================================================
// Auto-generated checksum: FNV-1a hash of {} directives + {} clauses = 0x{:016X}
// If this doesn't match c_api.rs, rebuild with `cargo clean && cargo build`
#define ROUP_CONSTANTS_CHECKSUM 0x{:016X}
