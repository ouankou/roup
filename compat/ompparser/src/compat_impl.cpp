...
static std::unordered_map<const OpenMPDirective*, std::string> g_plain_directives;
...
OpenMPDirective* parseOpenMP(const char* input, void* exprParse(const char* expr)) {
    ...
    const char* plain = roup_directive_plain_string(roup_dir);
    if (plain) {
        g_plain_directives[dir] = std::string(plain);
    } else {
        g_plain_directives[dir] = std::string();
    }
...
const char* getPlainDirective(const OpenMPDirective* dir) {
    auto it = g_plain_directives.find(dir);
    if (it == g_plain_directives.end()) {
        return nullptr;
    }
