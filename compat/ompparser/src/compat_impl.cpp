...
// Cache sanitized directive templates per compatibility directive. We retain the
// strings for the lifetime of the compatibility directive so repeated
// getPlainDirective() calls avoid reparsing through ROUP. Entries are kept
// indefinitely; the footprint is minimal because templates are short strings and
// directives are typically long-lived for tooling integrations.
static std::unordered_map<const OpenMPDirective*, std::string> g_plain_cache;
static std::mutex g_plain_cache_mutex;
...
OpenMPDirective* parseOpenMP(const char* input, void* exprParse(const char* expr)) {
    ...
    const char* plain = roup_directive_template(roup_dir);
    std::string plain_copy = plain ? std::string(plain) : std::string();

    {
        std::lock_guard<std::mutex> guard(g_plain_cache_mutex);
        g_plain_cache[dir] = plain_copy;
    }
...
const char* getPlainDirective(const OpenMPDirective* directive) {
    if (!directive) {
        return nullptr;
    }
    ...
    {
        std::lock_guard<std::mutex> guard(g_plain_cache_mutex);
        auto it = g_plain_cache.find(directive);
        if (it != g_plain_cache.end()) {
            cached = it->second;
            found = true;
        }
    }
    ...
    auto* mutable_dir = const_cast<OpenMPDirective*>(directive);
    std::string generated = mutable_dir->generatePragmaString();
    if (generated.empty()) {
        return nullptr;
    }

    OmpDirective* temp = roup_parse(generated.c_str());
    if (!temp) {
        return nullptr;
    }

    const char* plain = roup_directive_template(temp);
    buffer = plain ? std::string(plain) : std::string();

    {
        std::lock_guard<std::mutex> guard(g_plain_cache_mutex);
        g_plain_cache[directive] = buffer;
    }

    roup_directive_free(temp);
    return buffer.c_str();
}
